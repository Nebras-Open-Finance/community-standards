<route lang="yaml">
meta:
  title: API Hub Release Notes
</route>

<script setup lang="ts">
// Phase 5b-v — API Hub Release Notes per-year page. Ported from
// `docs/components/WebPages/APIHubReleaseNotesPage.vue`.
//
...
import {
  ...
  type ReleaseEntry,
  type ReleaseEndpoint,
  type ReleaseSectionRef,
...
// ─── Base set ────────────────────────────────────────────────────────────
const yearEntries = computed<ReleaseEntry[]>(() =>
  API_HUB_RELEASES
    .filter((e) => yearOf(e.release) === year.value)
    .slice()
    .sort((a, b) => {
...
const areaCountTotal = computed<number>(() => {
...
// ─── Facet state ─────────────────────────────────────────────────────────
type AudienceFilter = 'TPP' | 'LFI'
const AUDIENCE_OPTIONS: AudienceFilter[] = ['TPP', 'LFI']

const query = ref<string>('')
const audienceFilter = ref<AudienceFilter | null>(null)
const areaFilter = reactive(new Set<string>())
const sectionFilter = reactive(new Set<string>())
const searchInput = ref<HTMLInputElement | null>(null)

function sectionsOf(entry: ReleaseEntry): string[] {
  return (entry.sections ?? []).map((s) => s.label)
}

function haystack(entry: ReleaseEntry): string {
  const parts: string[] = [
    entry.title,
    entry.summary,
    entry.description,
...
// ─── Predicates ──────────────────────────────────────────────────────────
function matchesAudience(entry: ReleaseEntry, audience: AudienceFilter | null): boolean {
  if (!audience) return true
  return entry.audience === audience || entry.audience === 'Both'
}

function matchesAreas(entry: ReleaseEntry, areas: ReadonlySet<string>): boolean {
  if (areas.size === 0) return true
  return entry.areas.some((a) => areas.has(a))
}

function matchesSections(entry: ReleaseEntry, sections: ReadonlySet<string>): boolean {
  if (sections.size === 0) return true
  return sectionsOf(entry).some((s) => sections.has(s))
}

function matchesQuery(entry: ReleaseEntry, q: string): boolean {
  if (!q) return true
  return haystack(entry).includes(q)
}

const filteredEntries = computed<ReleaseEntry[]>(() => {
...
function audienceCount(value: AudienceFilter): number {
...
function sectionPillCount(sec: string): number {
...
// ─── Toggles ─────────────────────────────────────────────────────────────
function toggleAudience(value: AudienceFilter): void {
...
// ─── URL helpers ─────────────────────────────────────────────────────────
function sectionUrl(ref: ReleaseSectionRef | ReleaseEndpoint): string {
...
function anchorFor(entry: ReleaseEntry): string { return anchorForEntry(entry) }

function hasChips(entry: ReleaseEntry): boolean {
  return entry.areas.length > 0
    || (entry.sections !== undefined && entry.sections.length > 0)
...
// ─── URL <-> state sync ─────────────────────────────────────────────────
function isAudience(value: string): value is AudienceFilter {
...
function writeToUrl(): void {
  ...
  if (audienceFilter.value) sp.set('audience', audienceFilter.value); else sp.delete('audience')
  if (areaFilter.size) sp.set('area', [...areaFilter].join(',')); else sp.delete('area')
  if (sectionFilter.size) sp.set('section', [...sectionFilter].join(',')); else sp.delete('section')
...
<template>
  <div v-if="yearEntries.length === 0" class="ed-ahr">
    <section class="ed-ahr-hero">
      <div class="ed-ahr-hero__inner">
        <h1 class="ed-ahr-hero__title">No releases recorded for {{ year }}</h1>
        <p class="ed-ahr-hero__sub">
          The API Hub release-notes register has no entries for <strong>{{ year }}</strong>.
          <router-link to="/tech/release-notes-and-erratas/">Back to Release Notes &amp; Erratas</router-link>.
        </p>
      </div>
    </section>
  </div>
  <div v-else class="ed-ahr">
