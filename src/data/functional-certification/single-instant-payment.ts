// Functional Certification config for the LFI (provider) side of Single Instant
// Payment — the first of the seven UAE Open Finance payment types. Each payment
// type is certified as its own standalone area (its own explainer, submission
// portal, and Service Desk ticket); the other six Multi-Payment variants follow
// this same shape. See src/data/functional-certification/types.ts (FcPaymentArea)
// for the reasoning.
//
// A Single Instant Payment settles on one of three execution modes — AANI (the
// primary instant rail), intra-bank (both legs at the LFI), or UAEFTS (the
// fallback). The terminal Open Finance status differs per mode, and the LFI —
// the only party that sees the raw rail outcome — PATCHes that terminal status to
// the Consent Manager Payment Log. This config drives the evidence the LFI
// assembles: rails supported and their terminal statuses, the AANI timing
// windows, Creditor decryption/validation and Risk handling, and the two
// consent-shape scenarios (account+balance reads before a payment, a refund read
// after one).

import type { FcPaymentArea } from './types'

const RS_BASE = 'https://rs1.{LFICODE}.apihub.openfinance.ae/open-finance/payment/{VERSION}'
const ACCOUNT_INFO_BASE =
  'https://rs1.{LFICODE}.apihub.openfinance.ae/open-finance/account-information/{VERSION}'
// The terminal-status PATCH is made against the API Hub Consent Manager Payment
// Log, not the resource server — the Consent Manager base is not version-pathed.
...
// The three rails a Single Instant Payment can execute on. Terminal statuses are
// the Open Finance values from the Payment Status page: AANI accepts without
// confirming credit posting (AcceptedWithoutPosting); intra-bank and UAEFTS both
// confirm the creditor side (AcceptedCreditSettlementCompleted). Only AANI, the
// primary instant rail, has its POST→rail and POST→terminal-PATCH delays timed.
const RAILS: FcPaymentArea['rails'] = [
  {
    key: 'aani',
    label: 'AANI',
...
export const singleInstantPaymentArea: FcPaymentArea = {
  kind: 'payment',
  ...
  authScreenScenarios: [
    ...
    },
    {
      ...
      guidance:
        'Authorise a payment whose Creditor name matches the account holder exactly. Screenshot your authorisation page showing the ConfirmationOfPayee.Yes indicator surfaced to the customer.',
    },
    {
      key: 'cop-partial',
      ...
      guidance:
        'Authorise a payment whose Creditor name is close but not exact. Screenshot your authorisation page showing the ConfirmationOfPayee.Partial indicator surfaced to the customer.',
    },
    {
      key: 'cop-no',
