...
The `CustomerFriendlyLogoUri` & `CustomerFriendlyName` allow TPPs to display a consistent user interface for end-users when selecting or authenticating with an Authorisation Server. This ensures the server is easily identifiable and trusted by customers.
...
Flags provide additional attributes about an Organisation or Authorisation Server.
...
The complete specification for API meta can be found:
