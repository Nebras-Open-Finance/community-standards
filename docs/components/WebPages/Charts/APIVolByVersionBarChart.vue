...
/* =========================
   GROUP BY VERSION
========================= */
const buildGroupedByVersion = (data) => {
  const map = {}

  data.forEach(item => {
    const version = getApiVersion(item.url)
    if (!version) return

    const volume = Number(item.Volume) || 0
    map[version] = (map[version] || 0) + volume
  })
...
const updateChart = () => {
  ...
  const { labels, values } = buildGroupedByVersion(filtered)
  ...
  if (chartInstance) {
    ...
    chartInstance = new Chart(canvasRef.value, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Open Finance API Calls (by Version)',
            data: values,
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: ctx =>
                `${ctx.parsed.y.toLocaleString()} calls`
            }
          }
        },
        scales: {
