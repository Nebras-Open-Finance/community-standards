// Single source of truth for API Hub release note entries.
// Renders via the APIHubReleaseNotesPage component, scoped per calendar year.
//
// To add a release:
//   1. Append entries below with the same `release` value (e.g. '2026.14.0'),
//      incrementing `number` per entry within that release.
//   2. If a new calendar year starts, create a thin shim at
//      docs/tech/release-notes-and-erratas/release-notes/api-hub/{year}.md
//      that just mounts <APIHubReleaseNotesPage year="{year}" />.
