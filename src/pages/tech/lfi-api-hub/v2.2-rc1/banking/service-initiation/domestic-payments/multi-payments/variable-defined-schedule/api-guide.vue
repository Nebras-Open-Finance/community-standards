...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          LFI &middot; Banking &middot; Service Initiation &middot; Multi-Payments &middot; Variable Defined Schedule
        </div>
        ...
        <p class="ed-doc__lede">
          Variable Defined Schedule lets a TPP initiate a series of domestic payments
          <strong>on a pre-agreed set of specific dates</strong>, where <strong>each scheduled entry
          carries its own amount</strong>, from a customer's account at your LFI via the API Hub. The
          TPP supplies an explicit schedule at consent time &mdash; each
          <code>PaymentExecutionDate</code> paired with the amount due on that date &mdash; and the
          customer authorises the full schedule once. On each scheduled date the TPP submits one payment
          without re-authorisation. Payments run on AANI as the primary rail with UAEFTS as the
          fallback. This guide covers the Ozone Connect endpoints your LFI MUST implement so the Hub
          can serve every scheduled payment under the consent from creation through to execution and
          status retrieval.
        </p>
        <p class="ed-doc__lede">
          ...
          <a href="./requirements">Variable Defined Schedule Requirements</a>. This guide covers the
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before implementing Variable Defined Schedule, ensure the following are in place:</EdProse>
      ...
      <EdBullets>
        ...
        <li><strong>Variable Defined Schedule advertised</strong> &mdash; <code>ApiMetadata.VariableDefinedSchedule.Supported</code> is set to <code>true</code> on your authorisation server entry in the Trust Framework.</li>
      </EdBullets>
    </EdSectionBand>
    ...
    >
      ...
      <EdProse>
        ...
        <a href="./requirements#consent-validation">Variable Defined Schedule Requirements &mdash; Consent Validation</a>.
      ...
      <EdProse>The decrypted consent-time PII for a Variable Defined Schedule consent looks like:</EdProse>
      ...
      <EdProse>
        For Variable Defined Schedule, <code>Initiation.Creditor</code> MUST be an array of exactly
        <strong>one</strong> entry &mdash; every scheduled payment under this consent pays the same
        creditor, only the amount per date varies. The full Creditor rules &mdash; cardinality, mandatory fields, BIC derivation
        &mdash; are in
    ...
    >
      ...
      <EdProse>
        ...
        a scheduled entry exists whose <code>PaymentExecutionDate</code> matches the submitted date
        <strong>and</strong> whose amount equals the submitted amount, confirms the entry has not
        yet been consumed, and validates the request against the OpenAPI schema &mdash; all before
        forwarding to your
        Ozone Connect
    ...
    >
      ...
      <EdProse>
        ...
        time the TPP submits a scheduled payment under an authorized Variable Defined Schedule consent.
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>paymentType</code></td><td>string</td><td>Yes</td><td>The payment type. MUST be <code>cbuae-payment</code> for domestic Variable Defined Schedule</td></tr>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>Instruction.Amount.Amount</code></td><td>string</td><td>Yes</td><td>Decimal amount with two fraction digits. MUST equal the amount on the matching scheduled entry for this <code>PaymentExecutionDate</code> &mdash; the Hub enforces this before forwarding</td></tr>
