...
export const data: RequirementsPageData = {
  ...
  preconditions:
    'All requests require an active <a href="/tech/tpp-standards/trust-framework/application">Trust Framework application</a> with the <strong>ISP</strong> role, a valid <a href="/tech/tpp-standards/trust-framework/certificates">transport certificate</a> presented on every request via mTLS, an active <a href="/tech/tpp-standards/security/fapi/message-signing">signing key</a> for JWT signing, and a <a href="/tech/tpp-standards/security/tokens/access-token">client credentials access token</a> with the <code>insurance</code> scope.',
  sections: [
    {
      id: 'post-quote',
      num: '01',
    ...
    },
    {
      ...
      rules: [
        ...
        { field: 'Webhook authentication', rule: 'Events are signed by the Hub. The TPP MUST verify the signature on every received event before acting on it — see <a href="/tech/tpp-standards/security/fapi/webhook-verification">Webhook Verification</a>.', validatedBy: 'TPP' },
