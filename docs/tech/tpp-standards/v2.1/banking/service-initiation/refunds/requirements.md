...
| 3 | Account state | The debtor account must be able to receive a refund credit transaction. <br> If the account is ineligible, the LFI returns `403` with `errorMessage`: `The debtor account is not in a state where it can receive a refund credit transaction.` <br> If the account is blocked from receiving payments, the LFI returns `403` with `errorMessage`: `The debtor account is blocked from receiving payments.` | LFI |
