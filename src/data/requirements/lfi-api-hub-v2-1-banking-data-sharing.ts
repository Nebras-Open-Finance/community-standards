      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['7', 'Range handling', 'A request whose <code>fromStatementDate</code> or <code>toStatementDate</code> extends beyond two years or lies in the future is well-formed and MUST NOT be rejected — return <code>200</code> with the matching subset (empty where none).'] },
            ...
            { cells: ['9', 'Invalid range', 'Reject the request only where it is genuinely malformed — an unparseable date, or a contradictory range where <code>fromStatementDate</code> is after <code>toStatementDate</code>. Return <code>400</code> with <code>errorCode</code>: <code>Resource.InvalidFormat</code>. The API Hub enforces the OpenAPI schema before proxying, so most format errors are rejected upstream.'] },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['9', 'Range handling', 'A request whose <code>fromBookingDateTime</code> or <code>toBookingDateTime</code> extends beyond two years or lies in the future is well-formed and MUST NOT be rejected — return <code>200</code> with the matching subset (empty where none).'] },
            ...
            { cells: ['11', 'Invalid range', 'Reject the request only where it is genuinely malformed — an unparseable date-time, or a contradictory range where <code>fromBookingDateTime</code> is after <code>toBookingDateTime</code>. Return <code>400</code> with <code>errorCode</code>: <code>Resource.InvalidFormat</code>. The API Hub enforces the OpenAPI schema before proxying, so most format errors are rejected upstream.'] },
            ...
            { cells: ['14', 'Account access', 'Validate that the account in the <code>accountId</code> path parameter is held by the PSU resolved from <code>o3-psu-identifier</code>. Apply the <a href="#account-access-validation">Account Access Validation</a> check before status handling — a non-held account MUST return <code>403</code> with <code>errorCode</code>: <code>Consent.PermanentAccountAccessFailure</code>.'] },
