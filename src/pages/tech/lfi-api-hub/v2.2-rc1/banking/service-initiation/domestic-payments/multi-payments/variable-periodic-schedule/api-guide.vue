...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          LFI &middot; Banking &middot; Service Initiation &middot; Multi-Payments &middot; Variable Periodic Schedule
        </div>
        <h1 class="ed-doc__title">
          Variable Periodic Schedule &mdash; API Guide
          <span class="ed-doc__read">14 min read</span>
        </h1>
        <p class="ed-doc__lede">
          Variable Periodic Schedule lets a TPP initiate a recurring series of domestic payments at
          a <strong>variable amount</strong> on a <strong>regular period</strong> (e.g. weekly,
          monthly, quarterly) from a customer's account at your LFI via the API Hub. The TPP supplies
          the period, the overall date range, and the per-period maximum amount at consent time, and
          the customer authorises the full periodic schedule once. In each period the TPP submits one
          payment without re-authorisation &mdash; the Hub enforces at-most-one execution per period
          and that the submitted amount does not exceed the consent-time per-period maximum. Payments
          run on AANI as the primary rail with UAEFTS as the fallback. This guide covers the Ozone
          Connect endpoints your LFI MUST implement so the Hub can serve every periodic payment under
          the consent from creation through to execution and status retrieval.
        </p>
        <p class="ed-doc__lede">
          ...
          <a href="./requirements">Variable Periodic Schedule Requirements</a>. This guide covers the
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before implementing Variable Periodic Schedule, ensure the following are in place:</EdProse>

      <EdBullets>
        <li><strong>API Hub onboarded</strong> &mdash; Your API Hub instance is provisioned and your <a href="/tech/lfi-api-hub/v2.2-rc1/api-hub/onboarding/environment-specific/">environment-specific configuration</a> is complete.</li>
        <li><strong>Enc1 key pair generated and registered</strong> &mdash; The TPP encrypts PII to your LFI's <a href="/tech/lfi-api-hub/v2.2-rc1/api-hub/onboarding/environment-specific/#enc1-encryption-key">Enc1 public key</a>. Your LFI MUST hold the corresponding private key and be able to look it up by <code>kid</code>.</li>
        <li><strong>Consent Journey implemented</strong> &mdash; The <a href="/tech/lfi-api-hub/v2.2-rc1/consent-journey/api-guide">Consent Journey API Guide</a> MUST be implemented first. A payment cannot be initiated without an authorized consent.</li>
        <li><strong>Ozone Connect connectivity verified</strong> &mdash; Bidirectional mTLS connectivity is confirmed between the API Hub and your Ozone Connect base URL. See <a href="/tech/lfi-api-hub/v2.2-rc1/api-hub/connectivity/">Connectivity &amp; Certificates</a>.</li>
        <li><strong>Variable Periodic Schedule advertised</strong> &mdash; <code>ApiMetadata.VariablePeriodicSchedule.Supported</code> is set to <code>true</code> on your authorisation server entry in the Trust Framework.</li>
      </EdBullets>
    </EdSectionBand>
    ...
    >
      ...
      <EdProse>
        ...
        <a href="./requirements#consent-validation">Variable Periodic Schedule Requirements &mdash; Consent Validation</a>.
      ...
      <EdProse>The decrypted consent-time PII for a Variable Periodic Schedule consent looks like:</EdProse>
      ...
      <EdProse>
        For Variable Periodic Schedule, <code>Initiation.Creditor</code> MUST be an array of exactly
        <strong>one</strong> entry &mdash; every periodic payment under this consent pays the same
        creditor, only the amount per period varies. The full Creditor rules &mdash; cardinality, mandatory fields, BIC derivation
        &mdash; are in
    ...
    >
      ...
      <EdProse>
        ...
        validates the access token, checks the consent is <code>Authorised</code>, checks that
        the period containing the <code>PaymentExecutionDate</code> has not already been consumed and
        that the submitted amount does not exceed the consent-time per-period maximum, and validates
        the request against the OpenAPI schema &mdash; all before forwarding to your
        Ozone Connect
    ...
    >
      ...
      <EdProse>
        ...
        time the TPP submits a periodic payment under an authorized Variable Periodic Schedule consent.
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>paymentType</code></td><td>string</td><td>Yes</td><td>The payment type. MUST be <code>cbuae-payment</code> for domestic Variable Periodic Schedule</td></tr>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>Instruction.Amount.Amount</code></td><td>string</td><td>Yes</td><td>Decimal amount with two fraction digits. MUST be at or below the consent-time per-period maximum &mdash; the Hub enforces this before forwarding</td></tr>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>paymentResponse.OpenFinanceBilling.numberOfSuccessfulTransactions</code></td><td>integer</td><td>No</td><td>Number of successful transactions (typically <code>1</code> per periodic payment)</td></tr>
