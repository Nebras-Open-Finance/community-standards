  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'prose', html: 'Once a user confirms the action, the change must take effect immediately — there must be no delay between confirmation and the consent reflecting its new state. <ul><li><strong>Revoke</strong> Immediate PATCH to <a href="/tech/tpp-standards/v2.2-draft/consent/open-api/patch-insurance-consents-ConsentId">/insurance-consents/{ConsentId}</a></li><li><strong>Pause</strong> No API Hub update — record paused state in the TPP\'s own system only</li><li><strong>Reactivate</strong> No API Hub update — clear the paused state in the TPP\'s own system only</li></ul>' },
      ],
    },
  ],
}
