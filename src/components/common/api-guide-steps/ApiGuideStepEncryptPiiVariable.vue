...
<template>
  <div class="ag-step">
    ...
    <EdNote type="info" title="Creditor array — choosing a beneficiary model">
      <p>
        <code>Initiation.Creditor</code> determines the beneficiary model for all payments made under
        this consent:
      </p>
      <EdRefTable>
        <table>
          <thead>
            <tr><th>Entries</th><th>Model</th><th>Effect</th></tr>
          </thead>
          <tbody>
            <tr><td>Omitted</td><td>Open beneficiary</td><td>No creditor fixed at consent time &mdash; each <code>POST /payments</code> supplies its own creditor</td></tr>
            <tr><td>1 entry</td><td>Single beneficiary</td><td>Consent is bound to that one recipient &mdash; every payment must go to that account</td></tr>
            <tr><td>2&ndash;10 entries</td><td>Multiple beneficiaries</td><td>Consent authorises any one of the listed accounts &mdash; each payment specifies which one</td></tr>
          </tbody>
        </table>
      </EdRefTable>
      <p>
        ...
        for the field schema, matching rules, and validation requirements.
      </p>
    </EdNote>
