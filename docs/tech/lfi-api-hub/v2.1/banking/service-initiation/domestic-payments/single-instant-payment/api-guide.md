...
| `o3-caller-software-statement-id` | Yes | Software statement ID of the TPP application |
| `o3-api-uri` | Yes | The parameterised URL of the API being called by the TPP |
| `o3-api-operation` | Yes | The HTTP method of the operation carried out by the TPP (`GET`) |
| `o3-ozone-interaction-id` | Yes | Hub-generated interaction ID. Equals `o3-caller-interaction-id` if the TPP provided one |
| `o3-consent-id` | Yes | The consentId for which this call is being made |
| `o3-psu-identifier` | Yes | Base64-encoded psuIdentifier JSON object |
| `o3-caller-interaction-id` | No | Interaction ID passed in by the TPP, if present |
...
| Parameter | Type | Description | Example |
|-----------|------|-------------|---------|
| `paymentId` | string | Identifier of the payment resource to retrieve | `5ff155ea-853f-480c-ac74-1eaed7c1201f` |
...
A successful `GET /payments/{paymentId}` returns `200 OK` with the payment resource in `data`.

#### `data`

| Field | Type | Required | Description | Example |
|-------|------|----------|-------------|---------|
| `id` | string | Yes | API-specific unique identifier for the payment resource. | `payment-123` |
| `consentId` | string | No | Unique consent identifier assigned by the TPP. | `cac2381a-7111-4c5f-bc2f-4319a93da7c5` |
| `paymentTransactionId` | string | No | End-to-end identifier generated by the underlying payment rails. | `txn-456789` |
| `status` | string | Yes | Current payment status. Allowed values: `Pending`, `AcceptedSettlementCompleted`, `AcceptedCreditSettlementCompleted`, `AcceptedWithoutPosting`, `Rejected`, `Received`. | `Pending` |
| `statusUpdateDateTime` | string | Yes | Date and time when the payment status was last updated. | `2023-01-01T12:00:00Z` |
| `rejectReasonCode[]` | array | No | List of rejection reasons, present only when the payment status is `Rejected`. Each object contains a standardized error code and a human-readable message. | — |
| `creationDateTime` | string | Yes | Date and time when the payment resource was created. | `2023-01-01T12:00:00Z` |
| `instruction` | object | No | The payment initiation payload. | — |
| `paymentPurposeCode` | string | Yes | The payment purpose code. | `ACM` |
| `debtorReference` | string | No | Reference shown on the debtor’s statement. | `Order-78910` |
| `openFinanceBilling` | object | No | Billing parameters associated with the payment. | — |

#### `rejectReasonCode[]` (optional) | `data.rejectReasonCode[]`

| Field | Type | Required | Description | Example |
|-------|------|----------|-------------|---------|
| `code` | string | Yes | Standardized error code identifying the reason for rejection. | `AANI.AM04` |
| `message` | string | Yes | Human-readable description of the rejection reason. | `Payment rejected due to insufficient funds.` |
...
| Field | Type | Required | Description | Example |
|-------|------|----------|-------------|---------|
| `Amount.amount` | string | Yes | Amount in decimal format. | `100.00` |
| `Amount.currency` | string | Yes | ISO-4217 currency code. | `AED` |
...
| Field | Type | Required | Description | Example |
|-------|------|----------|-------------|---------|
| `Type` | string | Yes | Billing type for the payment instruction. | `Collection` |
| `NumberOfSuccessfulTransactions` | integer | No | Number of successful transactions processed. | `1` |
| `MerchantId` | string | No | Merchant identifier. | `MERCHANT123` |
...
#### Example response — Rejected payment
...
Only return an error when the request is invalid or a server condition prevents processing. All error bodies must include `errorCode` and `errorMessage`.

##### `400` — Bad request

| `errorCode` | When to use |
|-------------|-------------|
| `Consent.Invalid` | Consent is invalid, not found or not associated with the given `paymentId` |
| `Consent.InvalidUserIdentifier` | Invalid user identifier in consent |
| `GenericRecoverableError` | Transient server error — Hub may retry after a delay |
| `GenericError` | Unrecoverable validation error not covered above |
...
```json
{
  "errorCode": "Consent.Invalid",
  "errorMessage": "Consent is invalid, not found or not associated with the given paymentId"
}
```
...
| `AccessToken.InvalidScope` | The Hub's token does not include the required scope |
| `Consent.Invalid` | Consent is invalid or expired |
| `GenericError` | Unrecoverable access error |
...
See the [GET /payments/{paymentId}](/tech/lfi-api-hub/v2.1/banking/service-initiation/open-api/payments-PaymentId) API reference for the full response schema.