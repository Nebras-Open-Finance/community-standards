...
const data = {
  ...
  sections: [
      ...
      blocks: [
        ...
        { kind: "table", table: {
          ...
          rows: [
            ...
            { cells: ["4", "<code>Initiation.Creditor</code>", '<code>Initiation.Creditor</code> is optional. If provided, it must contain 1–10 creditor entries. Validate that each creditor is a valid UAE domestic creditor — the account is reachable on a supported UAE domestic rail (AANI or UAEFTS) and, where the state of the receiving account can be determined, able to receive payments. Mandatory fields, IBAN, and BIC derivation rules apply to every entry — see <a href="/tech/tpp-standards/v2.1/banking/service-initiation/personal-identifiable-information/creditor">Creditor</a>. If any check fails, respond with <code>invalid</code>.'] },
            { cells: ["5", "Payment type not supported", "Delegated SCA must be advertised as supported for the requested beneficiary model via the corresponding flag on your authorisation server entry in the Trust Framework: <code>ApiMetadata.DelegatedAuthentication.SingleBeneficiarySupported</code> (exactly 1 creditor entry), <code>ApiMetadata.DelegatedAuthentication.MultipleBeneficiariesSupported</code> (2–10 entries), or <code>ApiMetadata.DelegatedAuthentication.OpenBeneficiariesSupported</code> (<code>Initiation.Creditor</code> omitted). If the LFI has not advertised support for the requested beneficiary model, respond with <code>invalid</code>."] },
    ...
    },
    {
      ...
      blocks: [
        { kind: "prose", html: 'When the TPP initiates a payment under an authorized consent, the API Hub validates the access token, consent status, amount/currency consistency with the consent, and OpenAPI schema before forwarding the request to your Ozone Connect <a href="/tech/lfi-api-hub/v2.1/banking/service-initiation/open-api/payments"><code>POST /payments</code></a> endpoint. The rules below cover what your Ozone Connect endpoint must validate on receipt. Error responses MUST conform to the <a href="/tech/lfi-api-hub/v2.1/banking/service-initiation/open-api/payments">POST <code>/payments</code> OpenAPI schema</a> — the <code>errorCode</code> values referenced below are drawn from the error schema matching the returned HTTP status (<code>Error400</code>, <code>Error403</code>, or <code>Error409</code>). Delegated SCA delegates strong customer authentication to the TPP, so the LFI MUST additionally verify the SCA proof delivered in the <code>Risk</code> block and the customer-present headers before executing the payment.' },
        { kind: "table", table: {
          headers: ["#", "Rule", "Detail"],
          rows: [
    ...
    },
    {
      ...
      blocks: [
        { kind: "prose", html: `Once the payment record has been created, the LFI MUST apply its standard fraud, sanctions, and AML screening controls before the payment is submitted to the domestic rail. These checks are the LFI's responsibility and follow the LFI's own policies and regulatory obligations. Screening may result in the payment being held, rejected, or referred for manual review after the <code>201</code> response has been returned to the TPP. The outcome MUST be reflected in the payment's <code>Status</code> and surfaced via <a href="/tech/lfi-api-hub/v2.1/banking/service-initiation/open-api/payments"><code>GET /payments/{paymentId}</code></a> so the TPP can observe the final state.` },
        { kind: "table", table: {
          headers: ["#", "Rule", "Detail"],
          rows: [
    ...
    },
    {
      ...
      blocks: [
        { kind: "prose", html: "Once the payment has passed screening, the LFI MUST submit it to the domestic rails for processing. AANI is the primary rail for Delegated SCA payments; UAEFTS is the fallback." },
        { kind: "table", table: {
          headers: ["#", "Rule", "Detail"],
          rows: [
            ...
            { cells: ["3", "Propagate status changes to the API Hub", `Whenever a rail status change at AANI or UAEFTS maps to a change in the payment's Open Finance status, the LFI MUST immediately call <a href="/tech/lfi-api-hub/v2.1/api-hub/consent-manager/open-api/payment-log-id"><code>PATCH /payment-log/{id}</code></a> on the API Hub Consent Manager to update <code>paymentResponse.status</code> to the new Open Finance status. Rail statuses that are deliberately not mapped (see <a href="/tech/lfi-api-hub/v2.1/banking/service-initiation/domestic-payments/overview/payment-status">Payment Status</a>) do not require a PATCH; the mapping document is the source of truth for which rail transitions are reportable.`] },
            { cells: ["4", "<code>paymentTransactionId</code> propagation", 'Once AANI or UAEFTS assigns the end-to-end identifier for the payment (the IPP identifier for AANI, or the equivalent for UAEFTS), the LFI MUST include it as <code>paymentTransactionId</code> on the next <a href="/tech/lfi-api-hub/v2.1/api-hub/consent-manager/open-api/payment-log-id"><code>PATCH /payment-log/{id}</code></a> — ideally on the same PATCH as the accompanying status transition. Once set, <code>paymentTransactionId</code> MUST NOT be changed. Note that <code>paymentTransactionId</code> identifies the payment instruction on the rail and is distinct from the Bank Data Sharing <code>transactionId</code> (which identifies a ledger entry).'] },
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "ed-req" }, _attrs))} data-v-5ec741e9><section class="ed-req-hero" data-v-5ec741e9><div class="ed-req-hero__inner" data-v-5ec741e9><div class="ed-req-hero__label" data-v-5ec741e9><span class="ed-req-hero__label-dash" data-v-5ec741e9></span> ${ssrInterpolate(unref(eyebrow))}</div><h1 class="ed-req-hero__title" data-v-5ec741e9>${ssrInterpolate(unref(data).title)} `);
      if (unref(data).version) {
        _push(`<span class="ed-req-hero__badge" data-v-5ec741e9>${ssrInterpolate(unref(data).version)}</span>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(data).readTime) {
        _push(`<span class="ed-req-hero__read" data-v-5ec741e9>${ssrInterpolate(unref(data).readTime)} read</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</h1><p class="ed-req-hero__sub" data-v-5ec741e9>${unref(data).lede ?? ""}</p>`);
      if (unref(data).preconditions) {
        _push(`<p class="ed-req-hero__sub ed-req-hero__sub--tight" data-v-5ec741e9>${unref(data).preconditions ?? ""}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></section><!--[-->`);
      ssrRenderList(unref(data).sections, (s, i) => {
        _push(ssrRenderComponent(_component_EdSectionBand, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (s.method && s.path) {
                _push2(`<div class="ed-req-endpoint" data-v-5ec741e9${_scopeId}><span class="${ssrRenderClass([methodClass(s.method), "http-badge"])}" data-v-5ec741e9${_scopeId}>${ssrInterpolate(s.method)}</span><code class="ed-req-endpoint__path" data-v-5ec741e9${_scopeId}>${ssrInterpolate(s.path)}</code></div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`<!--[-->`);
              ssrRenderList(s.callouts || [], (c, ci) => {
                _push2(`<div class="${ssrRenderClass([`ed-req-callout--${c.kind}`, "ed-req-callout"])}" data-v-5ec741e9${_scopeId}>`);
                if (c.title) {
                  _push2(`<div class="ed-req-callout__title" data-v-5ec741e9${_scopeId}>${ssrInterpolate(c.title)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`<div class="ed-req-callout__body" data-v-5ec741e9${_scopeId}>${c.html ?? ""}</div></div>`);
              });
              _push2(`<!--]--><!--[-->`);
              ssrRenderList(s.blocks || [], (b, bi) => {
                _push2(`<!--[-->`);
                if (b.kind === "prose") {
                  _push2(`<p class="ed-req-intro" data-v-5ec741e9${_scopeId}>${b.html ?? ""}</p>`);
                } else if (b.kind === "table") {
                  _push2(`<div class="ed-req-table ed-req-table--generic" role="table"${ssrRenderAttr("aria-label", s.title)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-5ec741e9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-5ec741e9${_scopeId}><!--[-->`);
                  ssrRenderList(b.table.headers, (h, hi) => {
                    _push2(`<div class="ed-req-cell" role="columnheader" data-v-5ec741e9${_scopeId}>${ssrInterpolate(h)}</div>`);
                  });
                  _push2(`<!--]--></div><!--[-->`);
                  ssrRenderList(b.table.rows, (r, ri) => {
                    _push2(`<div class="ed-req-row" role="row" data-v-5ec741e9${_scopeId}><!--[-->`);
                    ssrRenderList(r.cells, (c, ci) => {
                      _push2(`<div class="ed-req-cell ed-req-cell--generic" role="cell" data-v-5ec741e9${_scopeId}>${c ?? ""}</div>`);
                    });
                    _push2(`<!--]--></div>`);
                  });
                  _push2(`<!--]--></div>`);
              ...
              if (s.rules && s.rules.length) {
                _push2(`<div class="ed-req-table" role="table"${ssrRenderAttr("aria-label", s.title)} data-v-5ec741e9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-5ec741e9${_scopeId}><div class="ed-req-cell ed-req-cell--num" role="columnheader" data-v-5ec741e9${_scopeId}>#</div><div class="ed-req-cell ed-req-cell--field" role="columnheader" data-v-5ec741e9${_scopeId}>Field</div><div class="ed-req-cell ed-req-cell--rule" role="columnheader" data-v-5ec741e9${_scopeId}>Rule</div><div class="ed-req-cell ed-req-cell--validator" role="columnheader" data-v-5ec741e9${_scopeId}>Validated by</div></div><!--[-->`);
                ssrRenderList(s.rules, (r, idx) => {
                  _push2(`<div class="ed-req-row" role="row" data-v-5ec741e9${_scopeId}><div class="ed-req-cell ed-req-cell--num" role="cell" data-v-5ec741e9${_scopeId}>${ssrInterpolate(idx + 1)}</div><div class="ed-req-cell ed-req-cell--field" role="cell" data-v-5ec741e9${_scopeId}>${r.field ?? ""}</div><div class="ed-req-cell ed-req-cell--rule" role="cell" data-v-5ec741e9${_scopeId}>${r.rule ?? ""}</div><div class="ed-req-cell ed-req-cell--validator" role="cell" data-v-5ec741e9${_scopeId}><span class="${ssrRenderClass([validatorClass(r.validatedBy), "ed-req-validator"])}" data-v-5ec741e9${_scopeId}><span class="ed-req-validator__label" data-v-5ec741e9${_scopeId}>${ssrInterpolate(splitValidator(r.validatedBy).label)}</span>`);
                  if (splitValidator(r.validatedBy).detail) {
                    _push2(`<span class="ed-req-validator__detail" data-v-5ec741e9${_scopeId}>${ssrInterpolate(splitValidator(r.validatedBy).detail)}</span>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  _push2(`</span></div></div>`);
              ...
              if (s.table) {
                _push2(`<div class="ed-req-table ed-req-table--generic" role="table"${ssrRenderAttr("aria-label", s.title)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(s.table.headers) })}" data-v-5ec741e9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-5ec741e9${_scopeId}><!--[-->`);
              ...
              ssrRenderList(s.subsections || [], (sub, si) => {
                _push2(`<div class="ed-req-sub" data-v-5ec741e9${_scopeId}><h3 class="ed-req-sub__heading" data-v-5ec741e9${_scopeId}>${ssrInterpolate(sub.heading)}</h3><!--[-->`);
                ...
                ssrRenderList(sub.blocks || [], (b, bi) => {
                  _push2(`<!--[-->`);
                  if (b.kind === "prose") {
                    _push2(`<p class="ed-req-sub__intro" data-v-5ec741e9${_scopeId}>${b.html ?? ""}</p>`);
                  } else if (b.kind === "table") {
                    _push2(`<div class="ed-req-table ed-req-table--generic ed-req-table--sub" role="table"${ssrRenderAttr("aria-label", sub.heading)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-5ec741e9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-5ec741e9${_scopeId}><!--[-->`);
                    ssrRenderList(b.table.headers, (h, hi) => {
                      _push2(`<div class="ed-req-cell" role="columnheader" data-v-5ec741e9${_scopeId}>${ssrInterpolate(h)}</div>`);
                    });
                    _push2(`<!--]--></div><!--[-->`);
                ...
                ssrRenderList(sub.subsections || [], (sub2, s2i) => {
                  _push2(`<div class="ed-req-sub ed-req-sub--nested" data-v-5ec741e9${_scopeId}><h4 class="ed-req-sub__heading ed-req-sub__heading--nested" data-v-5ec741e9${_scopeId}>${ssrInterpolate(sub2.heading)}</h4><!--[-->`);
                  ...
                  ssrRenderList(sub2.blocks || [], (b, bi) => {
                    ...
                    if (b.kind === "prose") {
                      ...
                      _push2(`<div class="ed-req-table ed-req-table--generic ed-req-table--sub" role="table"${ssrRenderAttr("aria-label", sub2.heading)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-5ec741e9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-5ec741e9${_scopeId}><!--[-->`);
                      ssrRenderList(b.table.headers, (h, hi) => {
                        _push2(`<div class="ed-req-cell" role="columnheader" data-v-5ec741e9${_scopeId}>${ssrInterpolate(h)}</div>`);
                      });
                      _push2(`<!--]--></div><!--[-->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/pages/tech/lfi-api-hub/v2.1/banking/service-initiation/domestic-payments/multi-payments/delegated-sca/requirements.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const requirements = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-5ec741e9"]]);
export {
