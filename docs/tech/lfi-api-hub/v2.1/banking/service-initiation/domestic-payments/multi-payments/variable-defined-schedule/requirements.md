...
| 5 | Payment type not supported | Variable Defined Schedule must be advertised as supported via `ApiMetadata.VariableDefinedSchedule.Supported` on your authorisation server entry in the Trust Framework. If the LFI has not advertised support for this payment type, respond with `invalid`. |
...
The generic [Authorization requirements](/tech/lfi-api-hub/v2.1/consent-journey/authorization/requirements) apply to this journey. The rules below cover the additional account selection logic specific to Variable Defined Schedule.
...
Once the payment has passed screening, the LFI MUST submit it to the domestic rails for processing. AANI is the primary rail for Variable Defined Schedule payments; UAEFTS is the fallback.
