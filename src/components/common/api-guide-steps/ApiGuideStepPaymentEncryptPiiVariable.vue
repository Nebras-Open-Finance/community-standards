...
<template>
  <div class="ag-step">
    ...
    <EdNote type="danger" title="Creditor must match the consent beneficiary model">
      <p>The creditor supplied here must correspond to the beneficiary model set at consent time:</p>
      <ul>
        <li><strong>Single beneficiary</strong> (<code>Initiation.Creditor[]</code> had 1 entry): <code>CreditorAccount.SchemeName</code>, <code>CreditorAccount.Identification</code>, and <code>CreditorAccount.Name</code> must exactly match that entry. The LFI decrypts both PII tokens and compares them; any discrepancy results in rejection.</li>
        <li><strong>Multiple beneficiaries</strong> (2&ndash;10 entries): must exactly match one entry from the pre-approved list in the consent PII.</li>
        <li><strong>Open beneficiary</strong> (<code>Initiation.Creditor[]</code> omitted at consent): no consent-time match required &mdash; supply any valid creditor.</li>
      </ul>
      <p>
