...
5. **Fixed On Demand advertised** — `ApiMetadata.FixedOnDemand.Supported` is set to `true` on your authorisation server entry in the Trust Framework
...
For Fixed On Demand, `Initiation.Creditor` MUST be an array of exactly **one** entry — the single creditor that every payment under this consent will pay. The full Creditor rules — cardinality, mandatory fields, BIC derivation — are in [Creditor](/tech/lfi-api-hub/v2.1/banking/service-initiation/personal-identifiable-information/creditor). This section walks through the validation in code.
...
| `paymentType` | string | Yes | The payment type. MUST be `cbuae-payment` for domestic Fixed On Demand |
...
| `Instruction.Amount.Amount` | string | Yes | Decimal amount with two fraction digits, e.g. `100.00`. For Fixed On Demand this MUST equal the consent's `PeriodicSchedule.Amount` — the Hub enforces this before forwarding |
