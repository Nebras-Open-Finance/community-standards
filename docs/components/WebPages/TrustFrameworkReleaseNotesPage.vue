...
} from '../../.vitepress/trust-framework-releases-registry'
import { latestApiHubYear } from '../../.vitepress/release-notes-years'
...
// ─── Facet state ─────────────────────────────────────────────────────────
const query = ref('')
const categoryFilter = reactive(new Set())
...
const groupedResults = computed(() => {
  ...
  return [...groups.values()].map((g) => {
    ...
    for (const e of g.entries) {
      if (!bucketMap.has(e.category)) bucketMap.set(e.category, [])
      bucketMap.get(e.category).push(e)
    }
...
function readFromUrl() {
  ...
  if (cats) for (const c of cats.split(',').filter(Boolean)) {
    if (TRUST_FRAMEWORK_CATEGORIES.includes(c)) categoryFilter.add(c)
  }
}
...
function writeToUrl() {
  ...
  categoryFilter.size ? sp.set('category', [...categoryFilter].join(',')) : sp.delete('category')
