...
const firstPreset: Preset = PRESETS[0] as Preset
...
const currentFlow = computed<Flow>(() => {
  const f = FLOWS[inputs.flowId]
  // Fallback to the first flow if id is somehow unknown — keeps the type
  // non-null without changing observable behaviour (id stays valid in UI).
  return f ?? (FLOW_LIST[0] as Flow)
})

const activePresetId = computed<string | null>(() => {
  for (const p of PRESETS) {
    let match = true
    for (const k of Object.keys(p.inputs) as (keyof typeof p.inputs)[]) {
      if (p.inputs[k] !== (inputs as unknown as Record<string, unknown>)[k as string]) {
        match = false
        break
      }
    }
...
const result = computed<FlowResult | null>(() => {
...
function computePaymentFlow(flow: Flow, i: CalculatorInputs): FlowResult {
...
function computeDataFlow(_flow: Flow, i: CalculatorInputs): FlowResult {
...
function onFeeCardClick(event: MouseEvent, anchor: string): void {
