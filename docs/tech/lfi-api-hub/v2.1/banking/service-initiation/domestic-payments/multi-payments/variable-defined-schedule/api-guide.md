...
5. **Variable Defined Schedule advertised** — `ApiMetadata.VariableDefinedSchedule.Supported` is set to `true` on your authorisation server entry in the Trust Framework
...
The full set of validation rules — `standardVersion`, `Initiation.DebtorAccount`, `BaseConsentId`, `CurrencyRequest`, payment-type support, PII conformance, creditor checks — is enumerated in [Variable Defined Schedule Requirements — Consent Validation](./requirements#consent-validation). The two parts that need a code walkthrough are **decrypting the PII** and **validating the creditor**; both are covered below.
...
The decrypted consent-time PII for a Variable Defined Schedule consent looks like:
...
For Variable Defined Schedule, `Initiation.Creditor` MUST be an array of exactly **one** entry — every scheduled payment under this consent pays the same creditor, only the amount per date varies. The full Creditor rules — cardinality, mandatory fields, BIC derivation — are in [Creditor](/tech/lfi-api-hub/v2.1/banking/service-initiation/personal-identifiable-information/creditor). This section walks through the validation in code.
...
| `paymentType` | string | Yes | The payment type. MUST be `cbuae-payment` for domestic Variable Defined Schedule |
...
| `Instruction.Amount.Amount` | string | Yes | Decimal amount with two fraction digits. MUST equal the amount on the matching scheduled entry for this `PaymentExecutionDate` — the Hub enforces this before forwarding |
...
```json
{
  ...
  "request": {
    "Data": {
      ...
      "Instruction": {
        "Amount": { "Amount": "2750.00", "Currency": "AED" }
      },
...
| `paymentResponse.OpenFinanceBilling.numberOfSuccessfulTransactions` | integer | No | Number of successful transactions (typically `1` per scheduled payment) |
