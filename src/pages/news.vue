...
type FilterKey = 'all' | ArticleKind
...
const filters = computed<FilterEntry[]>(() => [
  { key: 'all', label: 'All articles', count: counts.all },
  ...ARTICLE_KINDS.filter((k) => (counts[k] || 0) > 0).map<FilterEntry>((k) => ({
    key: k,
...
const featured = computed<HomeArticle | null>(() =>
  sortOrder.value === 'newest' && activeFilter.value === 'all'
    ? filtered.value[0] ?? null
    : null,
)

const paginatableArticles = computed<HomeArticle[]>(() =>
...
const remaining = computed<HomeArticle[]>(() => {
...
const pageNumbers = computed<PageNumberItem[]>(() => {
  ...
  const windowSet = new Set<number>([1, total, cur - 1, cur, cur + 1])
  const pages = [...windowSet]
    .filter((n) => n >= 1 && n <= total)
    .sort((a, b) => a - b)
  const out: PageNumberItem[] = []
  let prev = 0
        ...
        <nav v-if="pageCount > 1" class="ed-pager" aria-label="Pagination">
          ...
          <span class="ed-pager__pages">
            <button
              ...
              :key="n.key"
              :class="['ed-pager__page', { 'is-active': n.type === 'page' && n.page === currentPage, 'is-gap': n.type === 'gap' }]"
              :disabled="n.type === 'gap'"
