...
export const data: RequirementsPageData = {
  ...
  version: 'v2.1',
  readTime: '6 min',
  lede: 'The tables below define the display, labelling, and behavioural requirements for the TPP Consent Management Interface (CMI). See the <a href="./user-experience">User Experience</a> page for interactive wireframes of the dashboard and detail pages.',
  ...
  sections: [
    ...
    },
    {
      ...
      title: 'Detail page',
      blocks: [
        { kind: 'prose', html: 'Selecting a consent on the dashboard opens its detail page. The detail page presents the same information the customer saw on the Consent Page at the time they gave consent — the permissions, limits, accounts, and conditions that defined what they agreed to. This gives the customer a clear reference point for what they originally authorised, without requiring them to recall it from memory. In addition to all fields shown on the dashboard card, the detail page must show a truncated Consent ID with a copy button (format: <code>f47ac10b...d479</code>).' },
      ],
      subsections: [
        {
        ...
        },
        {
          ...
          subsections: [
            {
              heading: 'Data Sharing — "How we are using your data" card',
              blocks: [
                { kind: 'prose', html: 'A card titled <strong>How we are using your data</strong> must appear below the Data permissions section for all Data Sharing consents, except when status is <code>Rejected</code>.' },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['Revoke (Data Sharing)', '<code>Stop Sharing</code>', 'Status is <code>AwaitingAuthorization</code>, <code>Authorized</code>, <code>Suspended</code>, or <code>Paused</code>'] },
            { cells: ['Revoke (Payment)', '<code>Cancel Permission</code>', 'Status is <code>AwaitingAuthorization</code>, <code>Authorized</code>, <code>Suspended</code>, or <code>Paused</code>'] },
          ],
        } },
    ...
    },
    {
      ...
      blocks: [
        { kind: 'prose', html: 'When the user selects Pause, Reactivate, or Revoke, replace the detail view with a <strong>single confirmation screen</strong> that includes: a title, a description of the impact of the action on the service (varying by use case), a Confirm button, and a Go back button.' },
        { kind: 'table', table: {
          headers: ['', 'Pause', 'Reactivate', 'Revoke — Data Sharing', 'Revoke — Payment'],
          rows: [
            { cells: ['<strong>Title</strong>', '<code>Pause data sharing</code> / <code>Pause payment permission</code>', '<code>Resume data sharing</code> / <code>Resume payment permission</code>', '<code>Stop sharing</code>', '<code>Cancel payment permission</code>'] },
            { cells: ['<strong>Confirm button</strong>', '<code>Confirm pause</code>', '<code>Confirm reactivation</code>', '<code>Confirm stop sharing</code>', '<code>Confirm cancellation</code>'] },
          ],
        } },
        { kind: 'prose', html: 'Once a user confirms the action, the change must take effect immediately — there must be no delay between confirmation and the consent reflecting its new state. For revocation an update to the API Hub, via a PATCH request must be made at the point of confirmation. <ul><li><strong>Revoke (Data Sharing)</strong> Immediate PATCH to <a href="/tech/tpp-standards/v2.1/consent/open-api/patch-account-access-consents-ConsentId">/account-access-consents/{ConsentId}</a></li><li><strong>Revoke (Payment)</strong> Immediate PATCH to <a href="/tech/tpp-standards/v2.1/consent/open-api/patch-payment-consents-ConsentId">/payment-consents/{ConsentId}</a></li><li><strong>Pause</strong> No API Hub update — record paused state in the TPP\'s own system only</li><li><strong>Reactivate</strong> No API Hub update — clear the paused state in the TPP\'s own system only</li></ul>' },
      ],
    },
  ],
}
