...
// Capabilities that do not exist in every version. Keep this list in step with
...
function dataDeletionApiRefs(base: string): SidebarItem[] {
  return ATTESTATION_CONSENTS.flatMap(([path, slug]) => [
...
export const buildTppSidebar = (version: Version): SidebarItem[] => [
  {
    ...
    items: [
      ...
      ...(hasDataDeletionConfirmation(version)
        ? [{
          ...
          collapsed: true,
          items: [
            { text: 'Overview', link: `${BASE}/${version}/consent/data-deletion-confirmation/` },
            {
              text: 'API Reference',
              collapsed: true,
              items: dataDeletionApiRefs(`${BASE}/${version}/consent/data-deletion-confirmation/open-api`),
            },
          ],
        }]
        : []),
...
/**
 * Reactive TPP Standards sidebar that follows the selected version. Unversioned
 * entries (trust framework, registration, security, certification) are shared
 * across versions and carry no version segment.
 */
export function useTppSidebar(): ComputedRef<SidebarItem[]> {
  const { selectedVersion } = useSelectedVersion()
  return computed(() => buildTppSidebar(selectedVersion.value))
}
