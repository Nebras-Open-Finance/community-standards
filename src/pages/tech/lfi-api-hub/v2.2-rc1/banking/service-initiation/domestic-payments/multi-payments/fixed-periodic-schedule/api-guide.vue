...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          LFI &middot; Banking &middot; Service Initiation &middot; Multi-Payments &middot; Fixed Periodic Schedule
        </div>
        ...
        <p class="ed-doc__lede">
          Fixed Periodic Schedule lets a TPP initiate a recurring series of domestic payments at a
          <strong>fixed amount</strong> on a <strong>regular period</strong> (e.g. weekly, monthly,
          quarterly) from a customer's account at your LFI via the API Hub. The TPP supplies the
          amount, the period, and the overall date range at consent time, and the customer authorises the
          full periodic schedule once. In each period the TPP submits one payment without
          re-authorisation &mdash; the Hub enforces at-most-one execution per period. Payments run on
          AANI as the primary rail with UAEFTS as the fallback. This guide covers the Ozone Connect
          endpoints your LFI MUST implement so the Hub can serve every periodic payment under the
          consent from creation through to execution and status retrieval.
        </p>
        <p class="ed-doc__lede">
          ...
          <a href="./requirements">Fixed Periodic Schedule Requirements</a>. This guide covers the
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before implementing Fixed Periodic Schedule, ensure the following are in place:</EdProse>
      ...
      <EdBullets>
        ...
        <li><strong>Fixed Periodic Schedule advertised</strong> &mdash; <code>ApiMetadata.FixedPeriodicSchedule.Supported</code> is set to <code>true</code> on your authorisation server entry in the Trust Framework.</li>
      </EdBullets>
    </EdSectionBand>
    ...
    >
      ...
      <EdProse>
        ...
        <a href="./requirements#consent-validation">Fixed Periodic Schedule Requirements &mdash; Consent Validation</a>.
      ...
      <EdProse>The decrypted consent-time PII for a Fixed Periodic Schedule consent looks like:</EdProse>
      ...
      <EdProse>
        For Fixed Periodic Schedule, <code>Initiation.Creditor</code> MUST be an array of exactly
    ...
    >
      ...
      <EdProse>
        ...
        matches the consent-time fixed amount, checks that the period containing the
        <code>PaymentExecutionDate</code> has not already been consumed, and validates the request
        against the OpenAPI schema &mdash; all before forwarding to your
        Ozone Connect
    ...
    >
      ...
      <EdProse>
        ...
        time the TPP submits a periodic payment under an authorized Fixed Periodic Schedule consent.
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>paymentType</code></td><td>string</td><td>Yes</td><td>The payment type. MUST be <code>cbuae-payment</code> for domestic Fixed Periodic Schedule</td></tr>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>Instruction.Amount.Amount</code></td><td>string</td><td>Yes</td><td>Decimal amount with two fraction digits. MUST equal the consent-time fixed amount &mdash; the Hub enforces this before forwarding</td></tr>
