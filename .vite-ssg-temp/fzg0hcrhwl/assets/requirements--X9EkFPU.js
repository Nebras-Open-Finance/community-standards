...
const data = {
  ...
  version: "v2.2-rc1",
  readTime: "6 min",
  lede: 'The <a href="/tech/tpp-standards/v2.2-rc1/consent/requirements">Consent requirements</a> and the <a href="./user-journeys">User Journeys</a> for this payment type also apply and must be adhered to.',
  preconditions: 'The tables below list the validation rules that apply to Single Instant Payment. The <strong>Validated by</strong> column indicates where each rule is enforced. All requests require an active <a href="/tech/tpp-standards/trust-framework/application">Trust Framework application</a> with the <strong>BSIP</strong> role, a valid <a href="/tech/tpp-standards/trust-framework/certificates">transport certificate</a> presented on every request via mTLS, and an active <a href="/tech/tpp-standards/security/fapi/message-signing">signing key</a> for JWT signing.',
  sections: [
    {
      id: "consent-creation",
      num: "01",
      ...
      rules: [
        ...
        { field: "OpenAPI schema", rule: 'The request must conform exactly to the <a href="/tech/tpp-standards/v2.2-rc1/consent/open-api/par">POST <code>/par</code> OpenAPI schema</a>. No additional or undocumented parameters are permitted.', validatedBy: "API Hub" },
        { field: "<code>consent.PersonalIdentifiableInformation</code>", rule: 'The decrypted PII payload must conform exactly to the <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/api-schema/pii-par">Domestic Payment PII Schema Object (<code>POST /par</code>)</a>. All required properties must be present with values of the correct type, and no additional or undocumented properties are permitted (<code>additionalProperties: false</code>).', validatedBy: "LFI (/consent/action/validate)" },
        { field: "<code>consent.PersonalIdentifiableInformation.Risk</code>", rule: `The <code>Risk</code> block must be fully populated — every field that is known or derivable from the TPP's system must be included. See <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/risk">Risk</a>.`, validatedBy: "Monitored by Nebras" },
        ...
        { field: "<code>Initiation.Creditor</code>", rule: 'Required. Must contain exactly one creditor entry. The Creditor must be a valid UAE domestic creditor — the account must be reachable on a supported UAE domestic rail (AANI or UAEFTS) and, where the LFI can determine the state of the receiving account, in a state able to receive payments. Mandatory fields, IBAN, and BIC derivation rules apply — see <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/creditor#validation-requirement">creditor field validation requirements</a>.', validatedBy: "LFI (/consent/action/validate)" },
        { field: "<code>consent.ControlParameters.ConsentSchedule.SinglePayment.Type</code>", rule: 'Must be <code>"SingleInstantPayment"</code>. <code>MultiPayment</code> and <code>FilePayment</code> must not be present.', validatedBy: "API Hub" },
        { field: "<code>consent.ControlParameters.ConsentSchedule.SinglePayment.Amount</code>", rule: "Required. Defines the exact amount for the payment.", validatedBy: "API Hub" },
        { field: "<code>consent.ControlParameters.ConsentSchedule.SinglePayment.Amount.Currency</code>", rule: "Must be <code>AED</code>.", validatedBy: "API Hub" },
        { field: "Consent (unsupported)", rule: 'The LFI must advertise Single Instant Payment as supported via <code>ApiMetadata.SingleInstantPayment.Supported</code> on its authorisation server entry in the <a href="/tech/tpp-standards/trust-framework/api-discovery">Trust Framework</a>. If the payment type is not supported, the consent validation will fail.', validatedBy: "LFI (/consent/action/validate)" },
        ...
        { field: "<code>consent.DebtorReference</code>", rule: "Where the reference is not dictated by the requirements of the payments use case, the TPP SHOULD set a <strong>Structured Reference</strong> that gives the User more context than a plain free-text value. To a maximum of 35 characters: the User-provided free-text reference (to a maximum of 22 characters); a dash (<code>-</code>) separator; the Creditor LFI bank name (to a maximum of 8 characters) where the payment is not initiated on behalf of a Merchant; a dash; optionally the Merchant name (to a maximum of 8 characters) where the TPP initiates on behalf of a Merchant, followed by a dash; and the TPP name, as found in the Trust Framework or a recognisable alternative. The value set here is fixed for the payment — <code>Data.DebtorReference</code> at Payment Initiation must match it exactly.", validatedBy: "TPP" },
    ...
    },
    {
      ...
      rules: [
        ...
        { field: "Request timing", rule: "The TPP MUST submit <code>POST /payments</code> without undue delay after completing the token exchange that follows the authorization callback. Although the access token is valid for 10 minutes, a <code>SingleInstantPayment</code> is initiated with the User actively present and awaiting confirmation — avoidable delay creates uncertainty about whether the payment has been initiated and degrades the User experience.", validatedBy: "TPP" },
        ...
        { field: "<code>Data.Instruction.Amount.Amount</code>", rule: "Must exactly match <code>consent.ControlParameters.ConsentSchedule.SinglePayment.Amount.Amount</code>.", validatedBy: "API Hub" },
        { field: "<code>Data.Instruction.Amount.Currency</code>", rule: "Must exactly match <code>consent.ControlParameters.ConsentSchedule.SinglePayment.Amount.Currency</code>.", validatedBy: "API Hub" },
        { field: "<code>Data.PaymentPurposeCode</code>", rule: "Must exactly match <code>consent.PaymentPurposeCode</code>.", validatedBy: "API Hub" },
        { field: "<code>Data.OpenFinanceBilling</code>", rule: "Must exactly match <code>consent.OpenFinanceBilling</code> (including <code>Type</code> and, if present, <code>MerchantId</code>).", validatedBy: "API Hub" },
        { field: "<code>Data.DebtorReference</code>", rule: "Must exactly match <code>consent.DebtorReference</code>.", validatedBy: "API Hub" },
        { field: "<code>Data.CreditorReference</code>", rule: "Must exactly match <code>consent.CreditorReference</code>.", validatedBy: "API Hub" },
        { field: "*(implicit)*", rule: "Only one payment may be made against this consent. A second <code>POST /payments</code> call will be rejected.", validatedBy: "API Hub" },
        { field: "OpenAPI schema", rule: 'The request must conform exactly to the <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/open-api/payments">POST <code>/payments</code> OpenAPI schema</a>. No additional or undocumented parameters are permitted.', validatedBy: "API Hub" },
        { field: "<code>PersonalIdentifiableInformation</code>", rule: 'The decrypted PII payload must conform exactly to the <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/api-schema/pii-payments">Domestic Payment PII Schema Object (<code>POST /payments</code>)</a>. All required properties must be present with values of the correct type, and no additional or undocumented properties are permitted (<code>additionalProperties: false</code>). Note that <code>DebtorAccount</code> is not part of the payment-time PII — the debtor is fixed by the consent, and <code>Initiation.Creditor</code> is a single object rather than an array.', validatedBy: "LFI" },
        { field: "<code>PersonalIdentifiableInformation.Risk</code>", rule: `The <code>Risk</code> block must be fully populated — every field that is known or derivable from the TPP's system must be included. See <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/risk">Risk</a>.`, validatedBy: "Monitored by Nebras" },
        { field: "<code>PersonalIdentifiableInformation</code> (Creditor)", rule: '<code>Initiation.Creditor[]</code> had exactly 1 entry at consent time. The submitted creditor must exactly match that consent-time entry. See <a href="/tech/tpp-standards/v2.2-rc1/banking/service-initiation/personal-identifiable-information/creditor">Creditor</a>.', validatedBy: "LFI" },
        { field: "<code>x-fapi-interaction-id</code>", rule: "Should be included. Should be a valid UUID (RFC 4122). An invalid value will not cause a failure but tracing will not be possible.", validatedBy: "N/A" },
    ...
    },
    {
      ...
      blocks: [
        { kind: "prose", html: "The debtor account selected at consent authorization must still be in a state that permits payment initiation at the time <code>POST /payments</code> is called. If the account's status has changed since consent authorization, the LFI will respond with <code>403</code> according to the table below. The TPP MUST handle these responses and surface a suitable message to the User." },
        { kind: "table", table: {
          headers: ["Status", "Response"],
          rows: [
            { cells: ["<code>Active</code>", "Payment is accepted and processed."] },
            { cells: ["<code>Inactive</code>, <code>Dormant</code>, <code>Suspended</code>", "<code>403</code> with <code>errorCode</code>: <code>Consent.AccountTemporarilyBlocked</code> and <code>errorMessage</code>: <code>The account is temporarily blocked.</code>"] },
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "ed-req" }, _attrs))} data-v-e97ea4f9><section class="ed-req-hero" data-v-e97ea4f9><div class="ed-req-hero__inner" data-v-e97ea4f9><div class="ed-req-hero__label" data-v-e97ea4f9><span class="ed-req-hero__label-dash" data-v-e97ea4f9></span> ${ssrInterpolate(unref(eyebrow))}</div><h1 class="ed-req-hero__title" data-v-e97ea4f9>${ssrInterpolate(unref(data).title)} `);
      if (unref(data).version) {
        _push(`<span class="ed-req-hero__badge" data-v-e97ea4f9>${ssrInterpolate(unref(data).version)}</span>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(data).readTime) {
        _push(`<span class="ed-req-hero__read" data-v-e97ea4f9>${ssrInterpolate(unref(data).readTime)} read</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</h1><p class="ed-req-hero__sub" data-v-e97ea4f9>${unref(data).lede ?? ""}</p>`);
      if (unref(data).preconditions) {
        _push(`<p class="ed-req-hero__sub ed-req-hero__sub--tight" data-v-e97ea4f9>${unref(data).preconditions ?? ""}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></section><!--[-->`);
      ssrRenderList(unref(data).sections, (s, i) => {
        _push(ssrRenderComponent(_component_EdSectionBand, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              if (s.method && s.path) {
                _push2(`<div class="ed-req-endpoint" data-v-e97ea4f9${_scopeId}><span class="${ssrRenderClass([methodClass(s.method), "http-badge"])}" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(s.method)}</span><code class="ed-req-endpoint__path" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(s.path)}</code></div>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`<!--[-->`);
              ssrRenderList(s.callouts || [], (c, ci) => {
                _push2(`<div class="${ssrRenderClass([`ed-req-callout--${c.kind}`, "ed-req-callout"])}" data-v-e97ea4f9${_scopeId}>`);
                if (c.title) {
                  _push2(`<div class="ed-req-callout__title" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(c.title)}</div>`);
                } else {
                  _push2(`<!---->`);
                }
                _push2(`<div class="ed-req-callout__body" data-v-e97ea4f9${_scopeId}>${c.html ?? ""}</div></div>`);
              });
              _push2(`<!--]--><!--[-->`);
              ssrRenderList(s.blocks || [], (b, bi) => {
                _push2(`<!--[-->`);
                if (b.kind === "prose") {
                  _push2(`<p class="ed-req-intro" data-v-e97ea4f9${_scopeId}>${b.html ?? ""}</p>`);
                } else if (b.kind === "table") {
                  _push2(`<div class="ed-req-table ed-req-table--generic" role="table"${ssrRenderAttr("aria-label", s.title)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-e97ea4f9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-e97ea4f9${_scopeId}><!--[-->`);
                  ssrRenderList(b.table.headers, (h, hi) => {
                    _push2(`<div class="ed-req-cell" role="columnheader" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(h)}</div>`);
                  });
                  _push2(`<!--]--></div><!--[-->`);
                  ssrRenderList(b.table.rows, (r, ri) => {
                    _push2(`<div class="ed-req-row" role="row" data-v-e97ea4f9${_scopeId}><!--[-->`);
                    ssrRenderList(r.cells, (c, ci) => {
                      _push2(`<div class="ed-req-cell ed-req-cell--generic" role="cell" data-v-e97ea4f9${_scopeId}>${c ?? ""}</div>`);
                    });
                    _push2(`<!--]--></div>`);
                  });
                  _push2(`<!--]--></div>`);
              ...
              if (s.rules && s.rules.length) {
                _push2(`<div class="ed-req-table" role="table"${ssrRenderAttr("aria-label", s.title)} data-v-e97ea4f9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-e97ea4f9${_scopeId}><div class="ed-req-cell ed-req-cell--num" role="columnheader" data-v-e97ea4f9${_scopeId}>#</div><div class="ed-req-cell ed-req-cell--field" role="columnheader" data-v-e97ea4f9${_scopeId}>Field</div><div class="ed-req-cell ed-req-cell--rule" role="columnheader" data-v-e97ea4f9${_scopeId}>Rule</div><div class="ed-req-cell ed-req-cell--validator" role="columnheader" data-v-e97ea4f9${_scopeId}>Validated by</div></div><!--[-->`);
                ssrRenderList(s.rules, (r, idx) => {
                  _push2(`<div class="ed-req-row" role="row" data-v-e97ea4f9${_scopeId}><div class="ed-req-cell ed-req-cell--num" role="cell" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(idx + 1)}</div><div class="ed-req-cell ed-req-cell--field" role="cell" data-v-e97ea4f9${_scopeId}>${r.field ?? ""}</div><div class="ed-req-cell ed-req-cell--rule" role="cell" data-v-e97ea4f9${_scopeId}>${r.rule ?? ""}</div><div class="ed-req-cell ed-req-cell--validator" role="cell" data-v-e97ea4f9${_scopeId}><span class="${ssrRenderClass([validatorClass(r.validatedBy), "ed-req-validator"])}" data-v-e97ea4f9${_scopeId}><span class="ed-req-validator__label" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(splitValidator(r.validatedBy).label)}</span>`);
                  if (splitValidator(r.validatedBy).detail) {
                    _push2(`<span class="ed-req-validator__detail" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(splitValidator(r.validatedBy).detail)}</span>`);
                  } else {
                    _push2(`<!---->`);
                  }
                  _push2(`</span></div></div>`);
              ...
              if (s.table) {
                _push2(`<div class="ed-req-table ed-req-table--generic" role="table"${ssrRenderAttr("aria-label", s.title)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(s.table.headers) })}" data-v-e97ea4f9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-e97ea4f9${_scopeId}><!--[-->`);
              ...
              ssrRenderList(s.subsections || [], (sub, si) => {
                _push2(`<div class="ed-req-sub" data-v-e97ea4f9${_scopeId}><h3 class="ed-req-sub__heading" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(sub.heading)}</h3><!--[-->`);
                ...
                ssrRenderList(sub.blocks || [], (b, bi) => {
                  _push2(`<!--[-->`);
                  if (b.kind === "prose") {
                    _push2(`<p class="ed-req-sub__intro" data-v-e97ea4f9${_scopeId}>${b.html ?? ""}</p>`);
                  } else if (b.kind === "table") {
                    _push2(`<div class="ed-req-table ed-req-table--generic ed-req-table--sub" role="table"${ssrRenderAttr("aria-label", sub.heading)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-e97ea4f9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-e97ea4f9${_scopeId}><!--[-->`);
                    ssrRenderList(b.table.headers, (h, hi) => {
                      _push2(`<div class="ed-req-cell" role="columnheader" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(h)}</div>`);
                    });
                    _push2(`<!--]--></div><!--[-->`);
                ...
                ssrRenderList(sub.subsections || [], (sub2, s2i) => {
                  _push2(`<div class="ed-req-sub ed-req-sub--nested" data-v-e97ea4f9${_scopeId}><h4 class="ed-req-sub__heading ed-req-sub__heading--nested" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(sub2.heading)}</h4><!--[-->`);
                  ...
                  ssrRenderList(sub2.blocks || [], (b, bi) => {
                    ...
                    if (b.kind === "prose") {
                      ...
                      _push2(`<div class="ed-req-table ed-req-table--generic ed-req-table--sub" role="table"${ssrRenderAttr("aria-label", sub2.heading)} style="${ssrRenderStyle({ gridTemplateColumns: genericGridTemplate(b.table.headers) })}" data-v-e97ea4f9${_scopeId}><div class="ed-req-row ed-req-row--head" role="row" data-v-e97ea4f9${_scopeId}><!--[-->`);
                      ssrRenderList(b.table.headers, (h, hi) => {
                        _push2(`<div class="ed-req-cell" role="columnheader" data-v-e97ea4f9${_scopeId}>${ssrInterpolate(h)}</div>`);
                      });
                      _push2(`<!--]--></div><!--[-->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/pages/tech/tpp-standards/v2.2-rc1/banking/service-initiation/domestic-payments/single-instant-payment/requirements.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const requirements = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-e97ea4f9"]]);
export {
