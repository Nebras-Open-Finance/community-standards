    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'table', table: {
          headers: ['#', 'Field', 'Rule'],
          rows: [
            { cells: ['1', '<code>consent.Permissions[*].InsuranceType</code>', 'Only present policies of sectors listed in <code>consent.Permissions</code>. A consent that grants <code>Motor</code> and <code>Home</code> only surfaces policies in those two sectors — Travel and Health policies the PSU may also hold MUST NOT appear.'] },
            { cells: ['2', 'Five-year selection window', 'Within each consented sector, present every policy the PSU holds or has held in the last <strong>five years</strong>, regardless of current <code>PolicyStatus</code>. Historical policies (<code>Expired</code>, <code>Lapsed</code>, <code>Cancelled</code>, <code>Surrendered</code>, etc.) MUST be selectable so the PSU can share historical insurance history for switching, broking, and advisory use cases. Each row MUST clearly label the policy\'s current <code>PolicyStatus</code> so the PSU is not misled into thinking a lapsed policy is still active.'] },
            { cells: ['3', 'No eligible policies', 'If the authenticated PSU does not hold any policies within the five-year window across any of the consented sectors, PATCH the consent to <code>Rejected</code> and call <code>doFail</code> with <code>error</code>: <code>invalid_request</code> and <code>error_description</code>: <code>user_lacks_eligible_policies</code>. See <a href="/tech/lfi-api-hub/v2.1/consent-journey/authorization/requirements">Authorization requirements</a> for details.'] },
            ...
            { cells: ['5', '<code>insurancePolicyIds</code> patch', 'On successful authorization, PATCH the consent with <code>insurancePolicyIds</code> set to the array of <code>InsurancePolicyId</code> values the PSU selected. This array is the authoritative consented set used by every subsequent data sharing request — see <a href="#policy-access-validation">Policy Access Validation</a>.'] },
          ],
        } },
      ],
    },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['6', 'Policy access', 'Validate that the <code>InsurancePolicyId</code> in the path parameter is part of the consented set held by the PSU resolved from <code>o3-psu-identifier</code>. Apply the <a href="#policy-access-validation">Policy Access Validation</a> check before returning — an ID outside the consented set MUST return <code>403</code> with <code>errorCode</code>: <code>Consent.PermanentPolicyAccessFailure</code>.'] },
          ],
        } },
      ],
    },
