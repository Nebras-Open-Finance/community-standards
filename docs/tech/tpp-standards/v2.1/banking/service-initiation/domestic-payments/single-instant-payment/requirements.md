...
| 18 | `x-fapi-auth-date` | Must be sent. The customer must have been authenticated before payment initiation. Must be a valid HTTP-date (RFC 7231), e.g. `Tue, 11 Sep 2012 19:43:31 UTC`. | TPP |
| 19 | `x-fapi-customer-ip-address` | Must be sent. The customer is present at the point of payment initiation. Must be a valid IPv4 or IPv6 address. | TPP |
| 20 | `x-customer-user-agent` | Should be sent. Should reflect the user-agent of the customer's browser or device. | TPP |
