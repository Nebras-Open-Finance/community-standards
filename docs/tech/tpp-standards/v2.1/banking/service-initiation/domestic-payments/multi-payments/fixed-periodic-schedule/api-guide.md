...
// March payment
const { PaymentId: marPay } = await initiateFixedPeriodicPayment(access_token, consentId, encryptedPII)

// April payment (following month, using a refreshed access token)
const { PaymentId: aprPay } = await initiateFixedPeriodicPayment(refreshedToken, consentId, encryptedPII)
```
...
# March payment
mar_pay = initiate_fixed_periodic_payment(access_token, consent_id, encrypted_pii)

# April payment (following month, using a refreshed access token)
apr_pay = initiate_fixed_periodic_payment(refreshed_token, consent_id, encrypted_pii)
```
