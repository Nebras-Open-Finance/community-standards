...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          TPP · Banking · Service Initiation · Variable Periodic Schedule
        </div>
        ...
        <p class="ed-doc__lede">
          A Variable Periodic Schedule consent authorises a TPP to initiate <strong>one payment per
          period</strong> at a <strong>variable amount</strong> up to a defined ceiling. The user
          authorises once &mdash; approving a maximum per-payment amount and the recurring period
          &mdash; and the TPP submits one payment per period without requiring re-authorisation each
          time.
        </p>
        <p class="ed-doc__lede">
          Common use cases include variable monthly utility bills, periodic service charges, and
          TPP-managed savings sweeps where the amount differs each period but must stay within a
          pre-approved cap.
        </p>
      </div>
    </section>
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before initiating a Variable Periodic Schedule payment, ensure the following requirements are met:</EdProse>
    ...
    >
      <EdProse>
        ...
        <code>PeriodicSchedule.Type</code> to <code>"VariablePeriodicSchedule"</code>. Unlike Fixed
        Periodic Schedule, the amount is <strong>not fixed at consent time</strong> &mdash; each
        <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call specifies its own amount, subject to the
        <code>MaximumIndividualAmount</code> ceiling. Only one payment may be submitted per period.
      </EdProse>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>DebtorReference</code></td><td>string</td><td>Reference shown on the debtor's statement</td><td><code>Utility Bill</code></td></tr>
            <tr><td><code>CreditorReference</code></td><td>string</td><td>Reference shown on the creditor's statement</td><td><code>Utility Bill</code></td></tr>
          </tbody>
        </table>
      </EdRefTable>

      <h3 class="ed-doc__subhead">ControlParameters &mdash; Variable Periodic Schedule</h3>
      <EdProse>
        <code>ControlParameters.ConsentSchedule.MultiPayment</code> carries the control definition. Set
        <code>PeriodicSchedule.Type</code> to <code>"VariablePeriodicSchedule"</code>. Only <strong>one
        payment is permitted per period</strong>. There is no <code>Controls</code> block &mdash; the
        period itself acts as the limiting boundary, and <code>MaximumIndividualAmount</code> caps
        each payment's value.
      </EdProse>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            <tr><td><code>MaximumCumulativeNumberOfPayments</code></td><td><strong>Yes</strong></td><td>Maximum total number of payments over the consent lifetime</td><td><code>12</code></td></tr>
            <tr><td><code>MaximumCumulativeValueOfPayments.Amount</code></td><td>No</td><td>Maximum total value of all payments over the consent lifetime</td><td><code>5000.00</code></td></tr>
      ...
      <EdProse>
        <strong>Periodic Schedule Parameters</strong> &mdash; define the amount ceiling and period, set
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>PeriodicSchedule.MaximumIndividualAmount.Amount</code></td><td>Yes</td><td>The maximum amount permitted for a single payment. Each <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call must be &le; this value</td><td><code>200.00</code></td></tr>
            <tr><td><code>PeriodicSchedule.MaximumIndividualAmount.Currency</code></td><td>Yes</td><td>ISO 4217 currency code</td><td><code>AED</code></td></tr>
          </tbody>
        </table>
      </EdRefTable>
      ...
      <EdNote type="info" title="One payment per period">
        <p>
          ...
          <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call within the same period, even if the amount is within the
          ceiling.
        </p>
      </EdNote>
    ...
    >
      ...
      <EdBullets>
        ...
        <li>The <code>MaximumIndividualAmount</code> ceiling (e.g. "up to AED 200.00 per payment").</li>
      ...
      <EdNote type="tip" title="User Experience">
        <p>
          ...
          Periodic Schedule <strong>Consent</strong> and <strong>Authorization</strong> pages the user
          sees at the bank.
        </p>
      </EdNote>
    </EdSectionBand>
    ...
    >
      ...
      <EdProse>
        Submit <strong>one payment per period</strong> under this consent. Each call specifies the
        actual amount for that period &mdash; it must be &le; <code>MaximumIndividualAmount</code>.
        The API Hub will reject a second payment submitted within the same period and any payment that
        exceeds the ceiling.
      </EdProse>
      ...
      <EdNote type="warning" title="Only one payment per period">
        <p>
          The consent permits exactly one <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call per period. Submitting a
          second payment within the same period will be rejected, regardless of the amount. Plan your
          payment schedule so that one payment is submitted after each period boundary.
        </p>
      </EdNote>

      <EdCodeGroup :tabs="step9Tabs" />

      <EdNote type="warning" title="Amount ceiling and period enforcement">
        <p>
          The API Hub will reject a payment if <code>Instruction.Amount</code> exceeds
          <code>MaximumIndividualAmount</code>, if a payment has already been submitted in the current
