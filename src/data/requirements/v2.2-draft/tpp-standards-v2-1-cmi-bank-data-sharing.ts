  ...
  sections: [
    ...
    },
    {
      ...
      subsections: [
        ...
        },
        {
          ...
          blocks: [
            { kind: 'prose', html: 'When a consent carries a <code>BaseConsentId</code>, a <strong>List of Updates</strong> link must be shown on the detail page. Clicking this link navigates to a dedicated view that displays all consents related to the current consent through the same <code>BaseConsentId</code>. The related consents are retrieved via <a href="/tech/tpp-standards/v2.2-draft/consent/open-api/account-access-consents">GET /account-access-consents</a>, passing the <code>baseConsentId</code> query parameter. A consent is included in the list if: <ul><li>its <code>BaseConsentId</code> matches the current consent\'s <code>BaseConsentId</code>, <strong>or</strong></li><li>its <code>ConsentId</code> matches the current consent\'s <code>BaseConsentId</code></li></ul> The current consent must <strong>not</strong> appear in its own list of updates.' },
          ],
          subsections: [
            {
    ...
    },
    {
      ...
      blocks: [
        ...
        { kind: 'prose', html: 'Once a user confirms the action, the change must take effect immediately — there must be no delay between confirmation and the consent reflecting its new state. <ul><li><strong>Revoke</strong> Immediate PATCH to <a href="/tech/tpp-standards/v2.2-draft/consent/open-api/patch-account-access-consents-ConsentId">/account-access-consents/{ConsentId}</a></li><li><strong>Pause</strong> No API Hub update — record paused state in the TPP\'s own system only</li><li><strong>Reactivate</strong> No API Hub update — clear the paused state in the TPP\'s own system only</li></ul>' },
      ],
    },
  ],
}
