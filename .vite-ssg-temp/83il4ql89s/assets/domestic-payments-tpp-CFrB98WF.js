const PAY_BASE = "https://rs1.altareq1.sandbox.apihub.openfinance.ae/open-finance/payment/{VERSION}";
const AIS_BASE = "https://rs1.altareq1.sandbox.apihub.openfinance.ae/open-finance/account-information/{VERSION}";
...
const CONSENT_SCHEMA = "AEBankServiceInitiationRichAuthorizationRequestsV21.AEBankServiceInitiationAuthorizationDetailsProperties";
const RISK_SCHEMA = "AEBankServiceInitiationRichAuthorizationRequests.AERisk";
const JWE_PII = "eyJhbGciOiJSU0EtT0FFUC0yNTYiLCJlbmMiOiJBMjU2R0NNIn0.encryptedKey.iv.ciphertext.tag";
function consentSeed(controlParameters) {
  return {
...
function buildType(t) {
  const guide = t.isDelegatedSca ? `${SI_GUIDE}/multi-payments/delegated-sca/api-guide` : t.key === "single-instant-payment" ? `${SI_GUIDE}/single-instant-payment/api-guide` : `${SI_GUIDE}/multi-payments/${t.key}/api-guide`;
  return {
    key: t.key,
    label: t.label,
...
const CAPABILITIES = [
  {
    ...
    timing: "before",
    description: "The payment consent carries ReadAccountsBasic/Detail and ReadBalances, so you read the debtor account and its balance before initiating the payment.",
  ...
  },
  {
    ...
    label: "Refunds",
    timing: "after",
    description: "The payment consent carries ReadRefundAccount, so you retrieve the debtor refund-account details after the payment.",
