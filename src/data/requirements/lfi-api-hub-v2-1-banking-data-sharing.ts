  ...
  sections: [
    {
      ...
      blocks: [
        { kind: 'prose', html: 'When a TPP creates a consent, the API Hub calls your <a href="/tech/lfi-api-hub/v2.1/consent-events/open-api/validate"><code>POST /consent/action/validate</code></a> endpoint before the consent is created. You MUST validate the consent and respond with <code>status: "valid"</code> or <code>status: "invalid"</code>. If you respond with <code>invalid</code>, the API Hub will not create the consent and the TPP will receive an error. This validation runs before the PSU is involved — there is no authentication or authorization at this stage. The purpose is to reject consents early that your systems cannot fulfil. The field names in the rules below match the Ozone Connect <code>newConsent</code> payload the Hub delivers — <code>standardVersion</code> sits at the top level of the consent object; <code>BaseConsentId</code>, <code>AccountType</code>, <code>AccountSubType</code>, and <code>Permissions</code> sit under <code>consentBody.Data</code>.' },
        { kind: 'table', table: {
          headers: ['#', 'Rule', 'Detail'],
          rows: [
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['6', 'Account status', 'If the account is not <code>Active</code>, do not return the resource. Apply the <a href="#account-status-handling">Account Status Handling</a> mapping to return <code>403</code> with the corresponding <code>errorCode</code> and <code>errorMessage</code>.'] },
          ],
        } },
      ],
    },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['6', 'Data retention', 'The endpoint must support retrieval of at least the last two years of statements.'] },
            { cells: ['7', 'Empty result', 'If no statements exist within the requested range, return <code>200</code> with an empty <code>data</code> array. Do not return <code>404</code>.'] },
            { cells: ['8', 'Pagination', 'Pagination must be supported. <code>meta.paginated</code>, <code>meta.totalRecords</code>, and <code>meta.totalPages</code> must be accurate.'] },
            { cells: ['9', '<code>AccountSubType</code>', 'Supported for all account subtypes: <code>CurrentAccount</code>, <code>Savings</code>, <code>CreditCard</code>, <code>Finance</code>, and <code>Mortgage</code>.'] },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['8', 'Data retention', 'The endpoint must support retrieval of at least the last two years of transactions.'] },
            { cells: ['9', 'Empty result', 'If no transactions exist within the requested range, return <code>200</code> with an empty <code>data</code> array. Do not return <code>404</code>.'] },
            ...
            { cells: ['12', 'Account status', 'If the account is not <code>Active</code>, do not return the resource. Apply the <a href="#account-status-handling">Account Status Handling</a> mapping to return <code>403</code> with the corresponding <code>errorCode</code> and <code>errorMessage</code>.'] },
          ],
        } },
      ],
    },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['2', 'Data completeness', 'All product data that exists or is derivable from your systems must be returned, including fees, charges, rates, rewards, benefits, and eligibility criteria where held.'] },
            { cells: ['3', '<code>FinanceRates</code> format', '<code>FinanceRates</code> may be returned as either a cleartext <code>AEProductFinanceRates</code> JSON object or as a JWE compact serialisation string. Encryption is at the LFI\'s discretion.'] },
            { cells: ['4', '<code>FinanceRates</code> — JWE', 'If encrypting <code>FinanceRates</code>, generate an ephemeral symmetric encryption key per response (must not be reused). Encrypt the <code>AEProductFinanceRates</code> payload as a JWE using the content encryption algorithm required by the Security Profile. Set <code>exp</code> to 30 minutes from the time of the response and set <code>kid</code> to the value of <code>x-fapi-interaction-id</code>. Transmit the encryption key to the User via an existing LFI channel (e.g. SMS or push notification) — do not include it in the API response.'] },
            { cells: ['5', 'Empty result', 'If no product data exists for the account, return <code>200</code> with an empty <code>data</code> array. Do not return <code>404</code>.'] },
            { cells: ['6', 'Pagination', 'Pagination must be supported. <code>meta.paginated</code>, <code>meta.totalRecords</code>, and <code>meta.totalPages</code> must be accurate.'] },
            { cells: ['7', '<code>AccountSubType</code>', 'Supported for all account subtypes: <code>CurrentAccount</code>, <code>Savings</code>, <code>CreditCard</code>, <code>Finance</code>, and <code>Mortgage</code>.'] },
            { cells: ['8', 'Account status', 'If the account is not <code>Active</code>, do not return the resource. Apply the <a href="#account-status-handling">Account Status Handling</a> mapping to return <code>403</code> with the corresponding <code>errorCode</code> and <code>errorMessage</code>.'] },
          ],
        } },
      ],
    },
      ...
      blocks: [
        { kind: 'table', table: {
          ...
          rows: [
            ...
            { cells: ['8', 'Pagination', '<code>meta.totalRecords</code> and <code>meta.totalPages</code> must be accurate.'] },
