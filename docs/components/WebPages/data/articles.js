/*
  Shared article feed. Drives NewsPage (full list + filters + pagination) and
  HomePage (top-N previews). Newest first.

  Each entry has:
    id        Stable slug, derived from title if not given
    link      External URL to the source article
    source    Display label for the publisher (rendered in meta row)
    kind      Category: 'press-release' | 'news' | 'regulation' | 'conference'
...
export const ARTICLE_KIND_LABELS = {
...
export const ARTICLE_KINDS = ['press-release', 'news', 'regulation', 'conference']
...
export const articles = [
  ...
  .map(a => ({
    ...a,
    id: a.link.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').slice(0, 90),
  }))

export function countByKind() {
  const counts = { all: articles.length }
  for (const k of ARTICLE_KINDS) counts[k] = 0
  for (const a of articles) counts[a.kind] = (counts[a.kind] || 0) + 1
  return counts
}
