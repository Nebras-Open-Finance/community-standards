...
// folder (ofp-001.outcome.vue / ofp-001.feedback.vue) — excluded from routing in
...
const outcomeMods = import.meta.glob('./ofp-001.outcome.vue', { eager: true }) as Record<string, { default: Component }>
const feedbackMods = import.meta.glob('./ofp-001.feedback.vue', { eager: true }) as Record<string, { default: Component }>
...
<template>
  <div class="ofp">
    ...
    <section class="ofp-band ofp-band--white">
      <div class="ofp-band__inner">
        <div class="ofp-band__head">
          <div class="ofp-band__eyebrow"><span class="ofp-band__eyebrow-dash" /> 04 · Draft schema</div>
          <h2 class="ofp-band__title">The array model, written out</h2>
          <p class="ofp-band__lede">
            Two working drafts attached to this proposal — the revised PAR consent object and the
            JSON-array create request — each opens in the same rendered view as the published API specs.
            Schemas the proposal changes are defined in full; schemas unchanged from the published spec are
            shown as stubs marked &ldquo;unchanged&rdquo;.
          </p>
        </div>
        ...
        <div class="ofp-cards">
          ...
          >
            ...
            <div class="ofp-card__meta">
              <span class="ofp-card__cat" :style="{ color: '#0043A6' }">PAR consent</span>
            </div>
            <h3 class="ofp-card__title">par-file-payment-schema.yaml</h3>
            <p class="ofp-card__desc">File Payment control parameters — file fields removed, a customer-facing Description added.</p>
          ...
          >
            ...
            <div class="ofp-card__meta">
              <span class="ofp-card__cat" :style="{ color: '#008B78' }">Bulk / Batch</span>
            </div>
            <h3 class="ofp-card__title">bulk-payment-schema.yaml</h3>
            <p class="ofp-card__desc">Payments carried as a JSON Instructions[] array in one signed request.</p>
            <div class="ofp-card__foot">
              <span class="ofp-card__cta">Open rendered schema</span>
