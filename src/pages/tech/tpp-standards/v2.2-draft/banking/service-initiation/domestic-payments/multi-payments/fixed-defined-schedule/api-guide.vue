...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          TPP · Banking · Service Initiation · Fixed Defined Schedule
        </div>
        ...
        <p class="ed-doc__lede">
          A Fixed Defined Schedule consent authorises a TPP to initiate payments on a
          <strong>pre-agreed set of specific dates</strong>, each with a <strong>fixed exact
          amount</strong>. Rather than a recurring period, the TPP supplies an explicit schedule at
          consent time &mdash; listing each <code>PaymentExecutionDate</code> alongside the precise
          amount to be collected on that date. The user authorises once, approving the full schedule,
          and the TPP submits one payment per scheduled date without requiring re-authorisation.
        </p>
        <p class="ed-doc__lede">
          Common use cases include fixed instalment plans, structured loan repayments, and membership
          or subscription billing where both the dates and exact amounts are known upfront.
        </p>
        <p class="ed-doc__lede">
          Fixed Defined Schedule is the <strong>locked-amount variant</strong> of
          <a href="../variable-defined-schedule/api-guide">Variable Defined Schedule</a> &mdash; same
          schedule shape, but each entry carries an exact <code>Amount</code> rather than a
          <code>MaximumIndividualAmount</code> ceiling.
        </p>
      </div>
    </section>
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before initiating a Fixed Defined Schedule payment, ensure the following requirements are met:</EdProse>
    ...
    >
      <EdProse>
        ...
        <code>PeriodicSchedule.Type</code> to <code>"FixedDefinedSchedule"</code>. Unlike a Periodic
        Schedule, there is no recurring period &mdash; instead, the <code>Schedule</code> array lists
        each specific <code>PaymentExecutionDate</code> alongside the exact amount to be collected on
        that date. The TPP submits one <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> per scheduled entry for exactly the
        amount defined in the schedule.
      </EdProse>
      ...
      <h3 class="ed-doc__subhead">ControlParameters &mdash; Fixed Defined Schedule</h3>
      <EdProse>
        <code>ControlParameters.ConsentSchedule.MultiPayment</code> carries the control definition. Set
        <code>PeriodicSchedule.Type</code> to <code>"FixedDefinedSchedule"</code>. The
        <code>Schedule</code> array defines every execution date and its exact payment amount &mdash;
        the TPP submits a payment on each date for precisely the <code>Amount</code> specified in that
        entry.
      </EdProse>
      ...
      <EdRefTable>
        <table>
          <thead><tr><th>Field</th><th>Required</th><th>Description</th><th>Example</th></tr></thead>
          <tbody>
            <tr><td><code>PeriodicSchedule.Type</code></td><td>Yes</td><td>Must be <code>"FixedDefinedSchedule"</code></td><td><code>FixedDefinedSchedule</code></td></tr>
            ...
            <tr><td><code>PeriodicSchedule.Schedule[*].Amount.Amount</code></td><td>Yes</td><td>The exact amount to be collected on this date. The <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call must use this exact value.</td><td><code>500.00</code></td></tr>
            <tr><td><code>PeriodicSchedule.Schedule[*].Amount.Currency</code></td><td>Yes</td><td>ISO 4217 currency code</td><td><code>AED</code></td></tr>
          </tbody>
        </table>
      </EdRefTable>
    ...
    >
      ...
      <EdBullets>
        ...
        <li>The full payment schedule &mdash; each date alongside its exact amount.</li>
      ...
      <EdNote type="tip" title="User Experience">
        <p>
          See <a href="./user-journeys">User Experience</a> for screen mockups of the Fixed Defined
    ...
    >
      ...
      <EdProse>
        ...
        <code>PaymentExecutionDate</code>, call <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> with the amount defined
        for that entry in the schedule. The API Hub will reject any payment that does not match the
        scheduled amount, duplicates a date already paid, or is submitted after the consent has
        expired.
      </EdProse>
      ...
      <EdNote type="warning" title="Amount enforcement">
        <p>
          The API Hub will reject a payment if <code>Instruction.Amount</code> does not match the
          <code>Amount</code> defined for the corresponding <code>PaymentExecutionDate</code> in the
          schedule, if that date has already been paid, or if any lifetime cumulative cap has been
          reached.
        </p>
      </EdNote>
    </EdSectionBand>
