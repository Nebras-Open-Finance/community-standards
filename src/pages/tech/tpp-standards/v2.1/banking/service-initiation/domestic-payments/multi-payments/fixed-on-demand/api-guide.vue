...
<template>
  <div class="ed-doc">
    <section class="ed-doc__hero">
      <div class="ed-doc__inner">
        <div class="ed-doc__eyebrow">
          ...
          TPP · Banking · Service Initiation · Fixed On-Demand
        </div>
        ...
        <p class="ed-doc__lede">
          A Fixed On-Demand consent authorises a TPP to initiate <strong>multiple payments</strong> at a
          <strong>fixed amount</strong> over the lifetime of the consent. The user authorises once
          &mdash; approving a specific per-payment amount and periodic limits &mdash; and the TPP can
          then submit individual payments on-demand without requiring re-authorisation for each one.
        </p>
        <p class="ed-doc__lede">
          Common use cases include fixed-amount subscription billing, regular instalment collection,
          and scheduled membership fees where the charge is always the same.
        </p>
      </div>
    </section>
    ...
    <EdSectionBand
      ...
      tone="cream"
    >
      <EdProse>Before initiating a Fixed On-Demand payment, ensure the following requirements are met:</EdProse>
    ...
    >
      <EdProse>
        ...
        <code>urn:openfinanceuae:service-initiation-consent:v2.1</code>. The key difference from
        Variable On-Demand is that <code>PeriodicSchedule.Amount</code> is a <strong>mandatory fixed
        amount</strong> &mdash; every payment under this consent must be for exactly this value.
      </EdProse>
      ...
      <h3 class="ed-doc__subhead">ControlParameters &mdash; Fixed On-Demand</h3>
      <EdProse>
        <code>ControlParameters.ConsentSchedule.MultiPayment</code> carries the control definition. Set
        <code>PeriodicSchedule.Type</code> to <code>"FixedOnDemand"</code>. The payment amount
        <strong>is fixed at consent time</strong> via <code>PeriodicSchedule.Amount</code> &mdash;
        every <span class="endpoint"><span class="http-method http-method--post">POST</span><code>/payments</code></span> call under this consent must use exactly this amount.
      </EdProse>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>MaximumCumulativeNumberOfPayments</code></td><td>No</td><td>Maximum total number of payments over the consent lifetime</td><td><code>24</code></td></tr>
          </tbody>
        </table>
      </EdRefTable>
      ...
      <EdRefTable>
        <table>
          ...
          <tbody>
            ...
            <tr><td><code>PeriodicSchedule.Controls.MaximumCumulativeValueOfPaymentsPerPeriod.Amount</code></td><td>At least one of two</td><td>Maximum cumulative value of payments within the period. Must be greater than <code>Amount</code></td><td><code>450.00</code></td></tr>
            <tr><td><code>PeriodicSchedule.Controls.MaximumCumulativeValueOfPaymentsPerPeriod.Currency</code></td><td>At least one of two</td><td>ISO 4217 currency code</td><td><code>AED</code></td></tr>
            <tr><td><code>PeriodicSchedule.Controls.MaximumCumulativeNumberOfPaymentsPerPeriod</code></td><td>At least one of two</td><td>Maximum number of payments within the period</td><td><code>3</code></td></tr>
          </tbody>
        </table>
      </EdRefTable>
      ...
      <EdNote type="warning" title="At least one periodic control must be provided">
        <p>
          At least one of <code>MaximumCumulativeValueOfPaymentsPerPeriod</code> or
          <code>MaximumCumulativeNumberOfPaymentsPerPeriod</code> must be present inside
          <code>PeriodicSchedule.Controls</code>. Both may be provided. When
          <code>MaximumCumulativeValueOfPaymentsPerPeriod</code> is provided, it must be greater than
          <code>Amount</code>.
        </p>
      </EdNote>
    ...
    >
      ...
      <EdBullets>
        ...
        <li>Any periodic caps (per-period count and value limits).</li>
      ...
      <EdNote type="tip" title="User Experience">
        <p>
          See <a href="./user-journeys">User Experience</a> for screen mockups of the Fixed On-Demand
    ...
    >
      ...
      <EdProse>
        This step can be called <strong>multiple times</strong> under the same consent. Unlike Variable
        On-Demand, the <code>Instruction.Amount</code> must exactly match the fixed <code>Amount</code>
        defined in <code>PeriodicSchedule</code> &mdash; the API Hub will reject any payment where the
      ...
      <EdNote type="warning" title="Fixed amount enforcement">
        <p>
          The API Hub will reject a payment if <code>Instruction.Amount</code> does not exactly match
          the <code>PeriodicSchedule.Amount</code> from the consent, or if any periodic or lifetime
          cumulative cap has already been reached.
        </p>
      </EdNote>
    </EdSectionBand>
