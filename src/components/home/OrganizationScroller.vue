...
// Phase 5b-i — auto-scrolling logo strip, ported from
// `docs/components/WebPages/Components/OrganizationScroller.vue`.
//
// Fetches `/api/trust-framework.json` (a static asset under `public/api/`,
// today copied verbatim from the docs build — see migration plan §8). Filters
// to LFIs and TPPs only, sorts production participants first, hides logos
// that 404 by clearing `logoUri` on `<img>` error.
//
// Strict TS: `axios.get<TrustFrameworkOrg[]>` types the response, plus a
// `RawOrg` guard so a malformed response can't poison the render. SSR-safe:
// the fetch runs in `onMounted` so the client takes over after hydration.
...
const scrollerStyle = computed<CSSProperties>(() => ({
  ...
  ['--org-scroll-duration' as string]:
    `${Math.max(orgs.value.length, 1) * SECONDS_PER_ORG}s`,
}))
...
function onLogoError(org: DisplayOrg): void {
  org.logoUri = null
}

function isParticipant(o: RawOrg): boolean {
  return o.type === 'LFI' || o.type === 'TPP'
}

function toDisplayOrg(o: RawOrg): DisplayOrg | null {
...
onMounted(async () => {
  try {
    const { data } = await axios.get<unknown>('/api/trust-framework.json')
    const list: RawOrg[] = Array.isArray(data) ? (data as RawOrg[]) : []
    // Upstream is already active-only and deduped by id. Show LFIs and TPPs
    // (including LFIs acting as TPPs); skip Authority entries. Production
    // first so live participants lead the strip.
    const participants = list
      .filter(isParticipant)
      .slice()
      .sort(
        (a, b) =>
          Number(b.isProduction === true) - Number(a.isProduction === true),
      )
      .map(toDisplayOrg)
      .filter((x): x is DisplayOrg => x !== null)
    totalCount.value = participants.length
    orgs.value = participants
  } catch {
    // silent — strip is decorative
  }
})
</script>

<template>
  <section v-if="orgs.length" class="org-strip">
